...
public extension Encodable {
    func getJSONString() -> String {
        guard let jsonData = try? JSONEncoder().encode(self),
            ...
            fatalError("Failed to get JSON string from \(self)")
        }
        return jsonString
    }
}
...
public extension View {
    func preview(_ preview: Preview, printJSONString: Bool = false) -> some View {
        if printJSONString { print(preview.getJSONString()) }
        return previewDevice(preview.getDevice())
            .previewLayout(preview.isDevice() ? .device : .fixed(width: preview.width ?? 350, height: preview.height ?? 667))
            .previewColorScheme(preview.getColorScheme())
    }

    func preview(_ data: PreviewData, printJSONString: Bool = true) -> some View {
        if printJSONString { print(data.getJSONString()) }
    ...
    func previewColorScheme(_ value: ColorScheme) -> some View {
        return environment(\.colorScheme, value)
    }
